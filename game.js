...
laser.collidedEnemies.add(target);
 ...
 if (target.health <= 0) { handleTargetDestroyed(target); } } }
    ...
    // Si no hay fondo actual (solo al inicio del juego), configura el primero.
    if (!currentBgFar) {
        console.log(`Configurando fondo inicial en el índice: ${currentBackgroundIndex}`);
    ...
    // Calculamos el índice del SIGUIENTE fondo en el bucle
    const nextIndex = (currentBackgroundIndex + 1) % assets.backgroundsFar.length;
    console.log(`Preparando siguiente fondo para la transición. Índice: ${nextIndex}`);

    // Asignamos y pre-renderizamos los siguientes fondos
    ...
    // Activamos la transición y actualizamos el índice principal para el próximo ciclo
